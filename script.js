...
const showGame = () => {
  //Atvaizduoja zaidimo score, likusi laika
  nemat.forEach((item) => {
...
const rounduSkaiciuokle = () => {
  if (zmogusScore > kompScore) {
    ...
    kompRound++;
    document.querySelector(".kompiuterio-laimeti").innerHTML += ` ${
      roundas + 1
    },`;
  } else {
    zmogusRound++;
...
const atvaizdavimas = () => {
  document.querySelector(
    ".komp-rounds"
  ).innerHTML = ` Kompiuteris: ${kompRound}`;
  document.querySelector(
    ".zaidejas-rounds"
  ).innerHTML = `Žaidėjas: ${zmogusRound}`;
...
const ankstRound = () => {
  ...
  else return `Roundas baigėsi lygiosiomis su ${kompScore} `;
};
...
const kvadrClicked = () => {
  ...
  document.querySelector(".kvadratas").style.backgroundImage =
    "url('media/cat1.jpg')";
...
const laikmatis = () => {
  document.querySelector(
    ".laikmatis"
  ).innerHTML = `Laikas: ${laikas} sekundės `;
  if (laikas < 30) {
    laikas++;
  } else {
    laikas = 0;
...
const baigti = () => {
  ...
  clearInterval(intervalas);
  clearTimeout(kvadratoIntervalas);
  const kvadr = (document.querySelector(".kvadratas").style.visibility =
    "hidden");
  document.querySelector(".zaidejo-laimeti").innerHTML =
    "Žaidejo laimeti roundai: ";
  document.querySelector(".kompiuterio-laimeti").innerHTML =
    "Kompiuterio laimeti roundai: ";
  document.querySelector(".preitas-game").innerHTML = "Preitas roundas: ";
};
//Audio
const playSound = () => {
  let x = new Audio("media/miau.mp3");
  x.volume = 0.1;
  x.play();
};
